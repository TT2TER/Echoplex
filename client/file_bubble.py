...
class Message_bubble(QWidget):
    ...
    def __init__(self, sender_id,name,avatar_path, time, msg):
        ...
        content_rect = self.ui.message_bubble.fontMetrics().boundingRect(0, 0, 300 - (2 * self.padding), 0, Qt.TextWordWrap, self.msg)
        # 计算适合的宽度，考虑了内边距的影响
        content_width = min(content_rect.width() + (2 * self.padding), 300)+ (2 * self.padding)
        # 获取文本在上述宽度下的实际高度
