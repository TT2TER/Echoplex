...
def init():  # 将config文件内的信息读入到字典中
    f = open("face_detect\config.txt")
    global Total_face_num
    lines=f.readlines()
    Total_face_num = len(lines)
...
# 加载OpenCV人脸检测分类器Haar
face_cascade = cv2.CascadeClassifier("D:\Anaconda3-2022\envs\pytorch\Lib\site-packages\cv2\data\haarcascade_frontalface_default.xml")
...
def write_config():
    print("新人脸训练结束")
    f = open('face_detect\config.txt', "a")
    T = Total_face_num
    ...
    f = open('face_detect\config.txt', 'r+')
    flist = f.readlines()
    flist[0] = str(int(flist[0]) + 1) + " \n"
    f.close()

    f = open('face_detect\config.txt', 'w+')
    f.writelines(flist)
    f.close()
